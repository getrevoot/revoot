...
fn phase_instructions(phase: ReviewWorkerPhase, total_rounds: usize) -> &'static str {
    match phase {
        ReviewWorkerPhase::Planning => {
            "Investigate the assigned scope now. If diff.mode is inline_complete, the complete body and its evidence_id are already provided below - it is already delivered, so do not call read_diff for it, and cite that evidence_id directly; only call read_diff when diff.mode is manifest_only. Only one payload-returning tool call (read_diff, a search tool, get_rules, and similar) is honored per turn; extra ones in the same turn are rejected without result. Batch multiple pages within one read_diff call's reads array instead of issuing several read_diff calls. Planning allows up to two turns for this; call checkpoint_review with a bounded plan_summary once you have enough context, or by the second turn regardless."
        }
        ReviewWorkerPhase::Reviewing { round } if usize::from(round) < total_rounds => {
            "Investigate now and call submit_candidate_finding for any real, evidenced issue as soon as you have it - do not hold it back to submit later. coverage_requirements gives the exact action, path, hunk_id, and missing_pages still required. If diff.mode is inline_complete, the complete body and its evidence_id are already provided below - it is already delivered, so do not call read_diff for it, and cite that evidence_id directly; only call read_diff when diff.mode is manifest_only. Only one payload-returning tool call is honored per turn; extra ones in the same turn are rejected without result. Batch multiple pages within one read_diff call's reads array instead of issuing several read_diff calls. This round allows up to four turns for this; call checkpoint_review once you are done, or by the fourth turn regardless."
        }
        ReviewWorkerPhase::Reviewing { .. } => {
            "This is the final review round. Investigate now and call submit_candidate_finding for any real, evidenced issue as soon as you have it - do not hold it back to submit later. coverage_requirements gives the exact action, path, hunk_id, and missing_pages still required. If diff.mode is inline_complete, the complete body and its evidence_id are already provided below - it is already delivered, so do not call read_diff for it, and cite that evidence_id directly; only call read_diff when diff.mode is manifest_only. Only one payload-returning tool call is honored per turn; extra ones in the same turn are rejected without result. Batch multiple pages within one read_diff call's reads array instead of issuing several read_diff calls. This round allows up to four turns for this; call complete_group once you are done, or by the fourth turn regardless."
        }
