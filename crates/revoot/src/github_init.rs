...
impl Default for GitHubInitOptions {
    fn default() -> Self {
        Self {
            image: "ghcr.io/getrevoot/revoot:latest".to_owned(),
...
mod tests {
    ...
    fn workflow_is_head_bound_pinned_and_least_privilege() {
        ...
        assert!(workflow.contains("image: ghcr.io/getrevoot/revoot:latest"));
        assert!(workflow.contains("options: --user 0:0"));
        assert!(workflow.contains("chown 0:0 \"$GITHUB_WORKSPACE\""));
